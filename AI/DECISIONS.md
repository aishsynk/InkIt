...
## 2026-09-26 — Grouped toolbar order, InkIt logo and refined icons

- Decision: toolbar buttons follow one canonical grouped order: Draw (Cursor, Pen, Highlighter, Eraser, Colour) | Add (Shapes, Text, Step numbers) | Present (Laser, Spotlight, Zoom, Screenshot, Whiteboard) | Edit (Select, Undo, Redo, Clear) | Find (Search, All features, More, Collapse). A divider marks each change of group, and the expert switches (snap, multi-tool, turn toolbar, customise) sit at the right end. Presets are subsets in the same order. Settings migration v4 re-sorts saved layouts and keeps visibility.
